...
def _nome_do_email(email: str) -> str:
    """
    Extrai e formata o nome a partir do email institucional.
    'maria.souza@cps.sp.gov.br' → 'Maria Souza'
    """
    try:
        parte_local = email.split("@")[0]          # 'maria.souza'
        partes = parte_local.split(".")             # ['maria', 'souza']
        return " ".join(p.capitalize() for p in partes)  # 'Maria Souza'
    except Exception:
        return email
...
class AuthController:
    def __init__(self, main):
        ...
        # "Não Possui Cadastro?" → navega para tela de cadastro
    ...
    def _get_login_aluno(self):
        ...
        for sufixo in ('', '_2', '_3'):
            ...
            pg_nome = 'pg_loginaluno' if sufixo == '' else f'pg_loginaluno{sufixo}'
            if inp_email and inp_senha and pagina is getattr(w, pg_nome, None):
    ...
    def _cadastrar_prof(self):
        ...
        if not campo_email:
            QMessageBox.warning(w, "Atenção", "Campo de e-mail não encontrado.")
            return

        email_txt = campo_email.text().strip()

        if not email_txt:
            QMessageBox.warning(w, "Atenção", "Preencha o e-mail institucional.")
            return

        # Extrai o nome automaticamente do email (maria.souza@ → Maria Souza)
        nome = _nome_do_email(email_txt)

        # A senha é definida como 'senha1234+' por padrão pelo service
        dados, erro = cadastrar_prof(nome, email_txt)
        if erro:
            QMessageBox.warning(w, "Erro", erro)
            return
    ...
    def _trocar_senha_aluno(self):
        ...
        if not self.main.usuario_logado:
            return
        atual = getattr(w, 'input_loginaluno_2', None)
        nova  = getattr(w, 'input_senhaaluno_2', None)

        if not all([atual, nova]):
            QMessageBox.warning(w, "Atenção", "Widgets de troca de senha não encontrados.")
            return

        if not atual.text() or not nova.text():
            QMessageBox.warning(w, "Atenção", "Preencha todos os campos.")
            return
        ...
        QMessageBox.information(w, "Sucesso", "Senha alterada com sucesso!")
        atual.clear()
        nova.clear()
        self.main.ir_para(w.pg_tipo_jogo)
    ...
    def _trocar_senha_prof(self):
        ...
        if not self.main.usuario_logado:
            return
        atual = getattr(w, 'input_loginaluno_3', None)
        nova  = getattr(w, 'input_senhaaluno_3', None)
